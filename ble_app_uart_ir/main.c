...
#define DEVICE_NAME                     "Arturas_Nordic_UART"                               /**< Name of device. Will be included in the advertising data. */
...
uint8_t laststate = 0;
uint8_t b_state = 0;
uint8_t b_laststate = 0;
uint8_t counter_update_ble = 0;
...
// define a pin on which we want to generate the pwm
#define led_red 22


// define the period of the waves in Microseconds, this will be passed according to the configurations for pwm instance
#define FREQ_IN_US  26.315//10000



// a simple flag which will tell the state of the pwm either it is ready to change the duty cycle or not
static volatile bool ready_flag;

// a simple handler which will be called upon each pwm value is finished loading
void pwm_ready_handler(uint32_t pwm_id)
{
    ready_flag = true; // set the flag to true to show that a new value for duty cycle can be set
...
#define LED_INTERVAL APP_TIMER_TICKS(10)
//#define LED_INTERVAL1 APP_TIMER_TICKS(10000)
...
static void timers_init(void)
{
    ...
    //err_code = app_timer_create(&m_app_timer_id1,
    //                            APP_TIMER_MODE_SINGLE_SHOT,
    //                            app_timer_handler_button);
    //APP_ERROR_CHECK(err_code);
...
void sensor(){
    if(nrf_gpio_pin_read(SENSOR) == 0){
        state = 1;
    }
    else if(!(nrf_gpio_pin_read(SENSOR) == 0)){
        state = 0;
    }
...
static void read_sensor_data()
{
      ...
      uint32_t err_code;
      uint16_t length1;
      char ir_count_data[20];
      
      
      length1 = sprintf(ir_count_data, "%d", counter);
      err_code = ble_nus_data_send(&m_nus, ir_count_data, &length1, m_conn_handle);
...
int main(void)
{
    nrf_gpio_cfg_input(SENSOR,NRF_GPIO_PIN_PULLUP);
    ...
    err_code = app_timer_start(m_app_timer_id, LED_INTERVAL, NULL);
    ...
    //ret_code_t err_code; // a variable to hold err_code value
...
// here we have configured one channel, we can configure max up to two channels per pwm instance in this library, if we use two channels then we need to pass two pins
    app_pwm_config_t pwm_config = APP_PWM_DEFAULT_CONFIG_1CH(FREQ_IN_US, led_red);
...
// Initialize the pwm and pass it the intance, configurations and handler, we can also write NULL if we don't want to use the handler 
   ...
   app_pwm_enable(&m_pwm); // enable the pwm signal so that the pwm is started on the specified pin
    
    //err_code = app_timer_start(m_app_timer_id, LED_INTERVAL, NULL);
    while(app_pwm_channel_duty_set(&m_pwm, 0, 99) == NRF_ERROR_BUSY);
    //err_code = app_timer_start(m_app_timer_id, LED_INTERVAL, NULL);
    
    // Enter main loop.
    for (;;)
    {
        if(counter_update_ble)
      {
        counter_update_ble = 0;
        read_sensor_data();
      }
        idle_state_handle();
    }
